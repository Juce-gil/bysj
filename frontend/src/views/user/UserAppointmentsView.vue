      ...
      <div v-else-if="filteredAppointments.length" class="appointment-list">
        <article v-for="item in filteredAppointments" :key="item.id" class="appointment-card qh-panel--subtle">
          ...
          <div class="appointment-grid">
            <div><span>预约时间</span><strong>{{ item.intendedTime }}</strong></div>
            <div><span>面交地点</span><strong>{{ item.intendedLocation || '待协商' }}</strong></div>
            <div><span>备注</span><strong>{{ item.remark || '暂无备注' }}</strong></div>
            <div><span>关联商品</span><RouterLink :to="`/goods/${item.goodsId}`">查看商品详情</RouterLink></div>
          </div>
...
const stats = computed(() => {
  ...
  return [
    ...
    { label: '待确认', value: String(pending).padStart(2, '0'), tip: '等待卖家确认时间与地点' },
    { label: '已完成', value: String(completed).padStart(2, '0'), tip: '可作为历史成交记录展示' },
    { label: '已取消', value: String(cancelled).padStart(2, '0'), tip: '保留取消后的记录状态' },
  ];
});

const statusOptions = computed(() => [...new Set(appointments.value.map((item) => item.status))].map((value) => ({ value, label: getAppointmentStatusMeta(value).text })));
...
.appointment-list { display: grid; gap: 18px; }
.appointment-card { padding: 20px; }
.appointment-card__head { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; }
.appointment-card__head h3 { margin: 0 0 8px; color: var(--qh-text-primary); }
.appointment-card__head p { margin: 0; color: var(--qh-text-secondary); }
.appointment-grid { margin-top: 18px; display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px; }
.appointment-grid span { display: block; font-size: 12px; color: var(--qh-text-secondary); margin-bottom: 6px; }
.appointment-grid strong { color: var(--qh-text-primary); font-weight: 600; }
.appointment-card__actions { margin-top: 18px; display: flex; justify-content: flex-end; }
.state-panel { padding: 12px; }
@media (max-width: 960px) {
  .toolbar { width: 100%; flex-wrap: wrap; justify-content: flex-end; }
  .toolbar-select, .appointment-grid { width: 100%; grid-template-columns: 1fr; }
  .appointment-card__head { flex-direction: column; }
}
</style>
